...
class ENDERECO:
    ...
    def create_widgets(self):
        ...
        def pesquisa_tabela():
            ...
            cursor.execute("SELECT cod_endereco,cep,estado,cidade,bairro,logradouro,numero FROM endereco_funcionario WHERE status = TRUE and (cod_endereco=%s OR cep = %s OR logradouro LIKE %s OR estado = %s OR cidade = %s OR numero = %s OR bairro = %s) ",(pesquisa,pesquisa,f"%{pesquisa}%",pesquisa,pesquisa,pesquisa,pesquisa))
            consulta_tabela = cursor.fetchall()
